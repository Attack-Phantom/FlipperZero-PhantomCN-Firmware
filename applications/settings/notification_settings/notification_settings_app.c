...
static NotificationAppSettings* alloc_settings(void) {
    ...
    item = variable_item_list_add(
        app->variable_item_list, "LCD 对比度", CONTRAST_COUNT, contrast_changed, app);
    value_index =
    ...
    item = variable_item_list_add(
        app->variable_item_list, "LCD 背光", BACKLIGHT_COUNT, backlight_changed, app);
    ...
    item = variable_item_list_add(
        app->variable_item_list, "LED 亮度", BACKLIGHT_COUNT, led_changed, app);
    value_index = value_index_float(
    ...
    if(furi_hal_rtc_is_flag_set(FuriHalRtcFlagStealthMode)) {
        ...
        value_index = 0;
        variable_item_set_current_value_index(item, value_index);
        variable_item_set_current_value_text(item, "隐蔽");
    } else {
        item = variable_item_list_add(

...
static bool system_settings_custom_event_callback(void* context, uint32_t event) {
    ...
    case VarItemListIndexDebug:
        dialog_message_set_header(msg, "启用调试?", 64, 4, AlignCenter, AlignTop);
    ...
    case VarItemListIndexSleepMethod:
        dialog_message_set_header(msg, "禁用深度睡眠?", 64, 4, AlignCenter, AlignTop);
        dialog_message_set_text(
            msg,
    ...
    case VarItemListIndexHeapTrace:
        dialog_message_set_header(msg, "启用堆跟踪?", 64, 4, AlignCenter, AlignTop);
        dialog_message_set_text(
            msg,
            "将占用更多 RAM 并可能\n"
            "导致内存不足错误。\n"
...
SystemSettings* system_settings_alloc(void) {
    ...
    item = variable_item_list_add(
        app->var_item_list, "握持方向", COUNT_OF(hand_mode), hand_orient_changed, app);

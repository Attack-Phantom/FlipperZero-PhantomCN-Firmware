...
// Header
...
#ifdef HID_TRANSPORT_BLE
    elements_multiline_text_aligned(canvas, 17, 4, AlignLeft, AlignTop, "鼠标抖动器(隐蔽)");
#else
    elements_multiline_text_aligned(canvas, 10, 2, AlignLeft, AlignTop, "鼠标抖动器(隐蔽)");
#endif
    ...
    canvas_set_font(canvas, FontPrimary);
    elements_multiline_text(canvas, AlignLeft, 50, "按开始\n开始抖动");
    ...
    if(model->running) {
        elements_multiline_text_aligned(canvas, 91, 41, AlignLeft, AlignBottom, "停止");
    } else {
        elements_multiline_text_aligned(canvas, 91, 41, AlignLeft, AlignBottom, "开始");
    }

...
static void hid_mouse_clicker_draw_callback(Canvas* canvas, void* context) {
    ...
    elements_multiline_text_aligned(canvas, 17, 3, AlignLeft, AlignTop, "鼠标点击器");
    ...
    if(model->running) {
        elements_multiline_text_aligned(canvas, 86, 37, AlignLeft, AlignBottom, "停止");
    } else {
        elements_multiline_text_aligned(canvas, 86, 37, AlignLeft, AlignBottom, "开始");
    }
    ...
    elements_multiline_text_aligned(canvas, 13, 62, AlignLeft, AlignBottom, "退出");
}

...
void infrared_scene_edit_delete_on_enter(void* context) {
    ...
    if(edit_target == InfraredEditTargetButton) {
        dialog_ex_set_header(dialog_ex, "删除按钮?", 64, 0, AlignCenter, AlignTop);
        ...
        dialog_ex_set_header(dialog_ex, "删除遥控器?", 64, 0, AlignCenter, AlignTop);
        infrared_text_store_set(
            infrared,
            0,
            "%s\n with %zu buttons",
    ...
    dialog_ex_set_right_button_text(dialog_ex, "删除");

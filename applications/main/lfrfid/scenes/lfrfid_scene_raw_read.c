...
void lfrfid_scene_raw_read_on_enter(void* context) {
    ...
    popup_set_header(popup, "读取 ASK 中", 91, 16, AlignCenter, AlignTop);
    popup_set_text(popup, "请勿移动\n持续 5 秒", 91, 29, AlignCenter, AlignTop);
...
bool lfrfid_scene_raw_read_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == LfRfidEventReadError || event.event == LfRfidEventReadOverrun) {
            ...
            popup_set_header(popup, "原始读取错误!", 64, 0, AlignCenter, AlignTop);
            popup_set_text(
                popup, "可能由 SD\n卡问题\n导致", 0, 13, AlignLeft, AlignTop);
            ...
            if(!state->error) {
                if(state->is_psk) {
                    ...
                    popup_set_header(popup, "读取 PSK 中", 91, 16, AlignCenter, AlignTop);

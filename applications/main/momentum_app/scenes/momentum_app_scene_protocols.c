...
static void momentum_app_scene_protocols_file_naming_prefix_changed(VariableItem* item) {
    ...
    variable_item_set_current_value_text(item, value ? "后缀" : "前缀");
...
void momentum_app_scene_protocols_on_enter(void* context) {
    ...
    variable_item_set_current_value_text(
        item, momentum_settings.file_naming_prefix_after ? "后缀" : "前缀");
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        case VarItemListIndexSubghzExtend: {
            ...
            if(value) {
                ...
                dialog_message_set_header(msg, "确定吗?", 64, 4, AlignCenter, AlignTop);
                dialog_message_set_buttons(msg, "No", NULL, "Yes");
                dialog_message_set_text(
                    msg,
                    (event.event == VarItemListIndexSubghzBypass) ? "解锁 TX 至 300-350,\n"
                                                                    "387-467, 779-928 MHz\n"
                                                                    "请负责任使用，遵守\n"
                                                                    "当地法律" :
                                                                    "扩展 TX 至 281-361,\n"

...
void nfc_scene_save_confirm_on_enter(void* context) {
    ...
    dialog_ex_set_left_button_text(dialog_ex, "跳过");
    dialog_ex_set_right_button_text(dialog_ex, "保存");
    dialog_ex_set_header(dialog_ex, "保存密钥?", 64, 0, AlignCenter, AlignTop);
    dialog_ex_set_text(dialog_ex, "所有未保存数据将丢失", 64, 12, AlignCenter, AlignTop);
